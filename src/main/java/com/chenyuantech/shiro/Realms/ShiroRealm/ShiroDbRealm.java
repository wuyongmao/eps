...
public class ShiroDbRealm extends AuthorizingRealm {  
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection arg0) {  
        USERS user = (USERS) SecurityUtils.getSubject().getSession().getAttribute(ShiroDbRealm.SESSION_USER_KEY);  
...
//        info.addRole(user.getRole().trim());  
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(  
        ...
        // 把token转换成User对象  
    	USERS userLogin = tokenToUser((UsernamePasswordToken) authcToken);  
        // 验证用户是否可以登录  
        boolean ui = uSERSService.getUSERSByExample(userLogin);  
        if(ui == false)  
...
//      Object principal = ui.getUsername();  
    ...
    private USERS tokenToUser(UsernamePasswordToken authcToken) {  
        ...
        user.setACC(authcToken.getUsername());  
